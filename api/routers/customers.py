...
@router.post("/", response_model=schema.CustomerResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/{customer_id}", response_model=schema.CustomerResponse)
def get_customer(customer_id: int, db: Session = Depends(get_db)):
    return controller.read_one(db, customer_id)


@router.put("/{customer_id}", response_model=schema.CustomerResponse)
